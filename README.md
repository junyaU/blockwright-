...
`IR`（中間表現）を境に、**上流＝AI（言語・分類のみ）** と **下流＝決定論コード（座標・幾何・素材）** を分離する。これが設計の背骨（seam）。

```mermaid
flowchart LR
    Chat["💬 ゲーム内チャット"]

    subgraph UP["上流：AI（賢さ）— 言語・分類のみ"]
        AI["🧠 Claude<br/>claude.ts ・ pipeline/intent.ts ・ v5/interpret.ts"]
    end

    IR{{"📦 IR（中間表現）<br/>「何を」だけ／絶対座標を持たない"}}

    subgraph DOWN["下流：決定論コード（正確さ）— 座標・幾何・素材"]
        Build["⚙️ build(ir, origin)"]
    end

    MC["🟩 Minecraft Bedrock（WebSocket）"]

    Chat --> AI
    AI -->|"生成"| IR
    IR -->|"署名は固定（v0→v5）"| Build
    Build -->|"fill / setblock"| MC

    classDef seam fill:#ffe6a7,stroke:#d39e00,stroke-width:3px,color:#000;
    class IR seam;
```

- **IR が契約境界（seam）。** AI の賢さ（上流）とコードの正確さ（下流）をここで分離する。
