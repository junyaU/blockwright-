...
- **シーン合成／複数オブジェクトの同時管理**（「ロボットの隣に家」「星を持ったロボット」）。セッションは**単一の現在対象**のみ扱う。将来。
...
- **キャッシュキー**：subjectの正規化文字列（例 "robot"）。サイズ違いはバリアント名 or キー付加で区別。
...
| ID | 条件 |
|---|---|
| AC-45 | 「ロボット作って」2回目はライブラリから即・決定論で建つ（再生成しない） |
